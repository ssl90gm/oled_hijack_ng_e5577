...
void put_rect(uint8_t x, uint8_t y, uint8_t w, uint8_t h, uint8_t red, uint8_t green, uint8_t blue) {
    int p_y, p_x;
    for (p_y = y; p_y < y + h; p_y += 1) {
        for (p_x = x; p_x < x + w; p_x += 1) {
...
void put_text(uint8_t x, uint8_t y, uint8_t w, uint8_t h,
    ...
    while(*text) {
        ...
        int letter_y, letter_x;
        for(letter_y = 0; letter_y < font_y_size; letter_y += 1) {
            for (letter_x = 0; letter_x < char_width; letter_x += 1) {
